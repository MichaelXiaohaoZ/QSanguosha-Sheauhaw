...
local function will_invoke_shyhchour(self)
...
local function player_chosen_shyhchour(self, targets)
...
sgs.ai_skill_use["@@shyhchour"] = function(self, prompt)
	if will_invoke_shyhchour(self) then
		local to_discard = self:askForDiscard("shyhchour", 2, 2, false, true)
		if #to_discard == 2 then
			...
			local target = player_chosen_shyhchour(self, shu_generals)
			if target then
				return ("@ShyhchourCard=%d+%d->%s"):format(to_discard[1], to_discard[2], target:objectName())
			end
		end
	end
	return "."
...
sgs.ai_need_damaged.shyhchour = function(self, attacker, player)
	if player:hasLordSkill("shyhchour") then
		local victim
...
sgs.ai_card_intention.ShyhchourCard = function(self, card, from, tos)
