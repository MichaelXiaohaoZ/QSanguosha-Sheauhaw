...
void QSanInvokeSkillDock::update()
{
    if (!_m_buttons.isEmpty()) {
        ...
        if (!Config.value("UseOldButtons", false).toBool())
            rowLeft1 = G_DASHBOARD_LAYOUT.m_handCardArea.left() - 10;
        else
            rowLeft1 = G_DASHBOARD_LAYOUT.m_confirmButtonArea.left() - 2.4 * G_DASHBOARD_LAYOUT.m_confirmButtonArea.width();
