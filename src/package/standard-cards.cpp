...
void Slash::onEffect(const CardEffectStruct &card_effect) const
{
    ...
    if (this->tag["Fuji_tag"].toStringList().contains(effect.to->objectName()))
        effect.jink_num = 0;
    QVariantList damage_list = this->tag["SlashAddDamage"].toList();
    effect.intensify = damage_list.takeFirst().toInt();
    this->setTag("SlashAddDamage", QVariant::fromValue(damage_list));
...
bool Peach::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *) const
{
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (asked == "jink") {
            ...
            if (!player->isLocked(jink) && room->askForSkillInvoke(player, "eight_diagram")) {
...
void Collateral::onEffect(const CardEffectStruct &effect) const
{
    ...
    } else if (this->tag["Fuji_tag"].toStringList().contains(killer->objectName())) {
        if (source->isAlive() && killer->isAlive() && killer->getWeapon())
            source->obtainCard(killer->getWeapon());
    } else {
...
bool ExNihilo::targetFilter(const QList<const Player *> &targets, const Player *, const Player *) const
{
    return targets.isEmpty();
}
...
void Duel::onEffect(const CardEffectStruct &effect) const
{
    ...
    forever{
        if (!first->isAlive() || this->tag["Fuji_tag"].toStringList().contains(first->objectName())) break;
