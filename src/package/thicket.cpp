    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *caopi, QVariant &data) const
    {
        ...
        if (caopi->isAlive() && room->askForSkillInvoke(caopi, objectName(), data)) {
            ...
            else
            {
                QList<int> types;
                types << 1 << 2;
                room->broadcastSkillInvoke(objectName(), caopi, types);
            }
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (change.to != Player::Play || !player->hasSkipped(Player::Draw)) return false;
        foreach (ServerPlayer *xuhuang, room->getOtherPlayers(player)) {
            if (!TriggerSkill::triggerable(xuhuang)) continue;
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        foreach (ServerPlayer *zhurong, room->getOtherPlayers(player)) {
            ...
            if (room->isAllOnPlace(use.card, Player::PlaceTable)) {
...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *lusu, QVariant &) const
    {
        if (lusu->hasFlag("haoshi")) {
            ...
            bool used = room->askForUseCard(lusu, "@@haoshi!", "@haoshi", QVariant(), Card::MethodNone);
