...
{
    room->broadcastSkillInvoke("dahe");
...
void TanhuCard::use(Room *room, ServerPlayer *lvmeng, QList<ServerPlayer *> &targets) const
{
    ...
    if (success) {
        //room->broadcastSkillInvoke("tanhu");
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *liubei, QVariant &) const
    {
        ...
        if (no_basic == 0) {
            if (room->askForSkillInvoke(victim, "zhaolie_obtain", "obtain:" + liubei->objectName())) {
                room->broadcastSkillInvoke("zhaolie");
...
bool ShyhchourCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
    return targets.isEmpty() && to_select->getKingdom() == "shu" && to_select != Self;
}

void ShyhchourCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    CardMoveReason reason(CardMoveReason::S_REASON_GIVE, player->objectName(), victim->objectName(), "shyhchour", QString());
    room->obtainCard(victim, this, reason, false);
}

class ShyhchourViewAsSkill : public ViewAsSkill
{
public:
    ShyhchourViewAsSkill() : ViewAsSkill("shyhchour")
    {
        response_pattern = "@@shyhchour";
    }
    ...
    const Card *viewAs(const QList<const Card *> &cards) const
    {
        ...
        ShyhchourCard *card = new ShyhchourCard;
        card->addSubcards(cards);
        return card;
    }
};

class Shyhchour : public TriggerSkill
{
public:
    Shyhchour() : TriggerSkill("shyhchour$")
    {
        events << EventPhaseStart << DamageInflicted << Dying;
        frequency = Limited;
        limit_mark = "@hate";
        view_as_skill = new ShyhchourViewAsSkill;
    }
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == EventPhaseStart && player->getMark("xhate") == 0 && player->hasLordSkill("shyhchour")
            && player->getPhase() == Player::Start && player->getCards("he").length() > 1) {
            foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
                if (p->getKingdom() == "shu") {
                    room->askForUseCard(player, "@@shyhchour", "@shyhchour-give", -1, Card::MethodNone);
                    break;
                }
            }
        } else if (triggerEvent == DamageInflicted && player->hasLordSkill(this) && player->getMark("ShyhchourTarget") == 0) {
            ...
            newdamage.transfer_reason = "shyhchour";
...
class ShyhchourDraw : public TriggerSkill
{
public:
    ShyhchourDraw() : TriggerSkill("#shyhchour")
    {
    ...
    {
        DamageStruct damage = data.value<DamageStruct>();
        if (player->isAlive() && damage.transfer && damage.transfer_reason == "shyhchour")
            player->drawCards(damage.damage, "shyhchour");
        return false;
    }
};
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *daqiao, QVariant &data) const
    {
        if (triggerEvent == DamageCaused) {
            ...
            if (damage.card && damage.card->isKindOf("Slash")
                ...
                room->broadcastSkillInvoke(objectName(), daqiao, 1);
                LogMessage log;
                log.type = "#Anxian";
                log.from = daqiao;
                log.arg = objectName();
            ...
            if (use.card->isKindOf("Slash")) {
                ...
                if (room->askForCard(daqiao, ".", "@anxian-discard", data, objectName())) {
                    room->broadcastSkillInvoke(objectName(), daqiao, 2);
                    daqiao->setFlags("AnxianTarget");
        ...
        if (TriggerSkill::triggerable(simazhao) && triggerEvent == Damaged) {
            ...
            for (int i = 0; i < damage.damage; i++) {
                ...
                if (simazhao->isAlive() && damage.from && damage.from->isAlive() && !damage.from->isKongcheng()) {
                    ...
                    QList<int> willdiscards;
                    if (!langgu_discard.isEmpty())
                        willdiscards = room->askForCardsChosen(simazhao, damage.from, "h", objectName(), 0, langgu_discard.length(), true, Card::MethodNone, other);
                    ...
                    if (!willdiscards.isEmpty()) {
                        DummyCard *dummy = new DummyCard(willdiscards);
