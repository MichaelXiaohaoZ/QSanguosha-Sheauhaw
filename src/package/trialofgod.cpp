...
public:
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (use.from != player && use.to.contains(player) && player->hasFlag("TengyunDamaged") && use.card->getTypeId() != Card::TypeSkill) {
...
class TengyunRecord : public TriggerSkill
{
public:
    TengyunRecord() : TriggerSkill("#tengyun-record")
    {
        events << PreDamageDone << DamageComplete << EventPhaseStart;
        global = true;
    }
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &) const
    {
        if (triggerEvent == PreDamageDone) {
            if (room->getCurrent() && room->getCurrent()->getPhase() != Player::NotActive) {
                room->setPlayerFlag(player, "TengyunDamaged");
            }
        } else if (triggerEvent == EventPhaseStart) {
            if (player->getPhase() == Player::NotActive) {
                foreach (ServerPlayer *p, room->getAllPlayers()) {
                    room->setPlayerFlag(p, "-TengyunDamaged");
                }
            }
        }
        return false;
...
public:
    ...
    {
        CardUseStruct use = data.value<CardUseStruct>();
        if (use.card->isKindOf("Slash") && !use.card->hasFlag("slashDisableExtraTarget") && player->getPhase() != Player::NotActive) {
            ...
            foreach (ServerPlayer *p, room->getAlivePlayers()) {
                if (use.to.contains(p) || room->isProhibited(player, p, use.card) || p->getKingdom() == "god") continue;
    ...
    : Package("TrialOfGod")
{
    ...
    skills << new TengyunRecord << new BossZhongdu << new BossFuranSave << new BossJiding;
}
