...
XunzhiytCard::XunzhiytCard()
{
    target_fixed = true;
    mute = true;
}

void XunzhiytCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    int index = qrand() % 2 + 1;
    ...
    room->acquireSkill(source, "xunzhiyt", false);
    room->setPlayerFlag(source, "xunzhiyt");
}

class XunzhiytViewAsSkill : public ZeroCardViewAsSkill
{
public:
    XunzhiytViewAsSkill() :ZeroCardViewAsSkill("xunzhiyt")
    {
    }

    bool isEnabledAtPlay(const Player *player) const
    {
        return !player->hasFlag("xunzhiyt");
    }

    const Card *viewAs() const
    {
        return new XunzhiytCard;
    }
};

class Xunzhiyt : public TriggerSkill
{
public:
    Xunzhiyt() :TriggerSkill("xunzhiyt")
    {
        events << EventPhaseChanging;
        view_as_skill = new XunzhiytViewAsSkill;
    }
...
:Package("yitian")
{
    ...
    jiangboyue->addSkill(new Xunzhiyt);
