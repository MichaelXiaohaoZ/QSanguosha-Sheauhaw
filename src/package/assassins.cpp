...
void MixinCard::onEffect(const CardEffectStruct &effect) const
{
    ...
    room->broadcastSkillInvoke("mixin");
...
public:
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == EventPhaseStart && player->getPhase() == Player::Discard && player->askForSkillInvoke(this)) {
            ...
            room->broadcastSkillInvoke("cangni");
            player->turnOver();
            return false;
...
void DuyiCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    ...
    if (card->isBlack()) {
        ...
        log.arg = "duyi";
        room->sendLog(log);
        room->broadcastSkillInvoke("duyi");
    } else
        room->broadcastSkillInvoke("duyi");
...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        log.arg = "chizhong";
        room->sendLog(log);
        room->broadcastSkillInvoke("chizhong");

        return false;
    }
};

class NosFenxin : public TriggerSkill
{
public:
    NosFenxin() : TriggerSkill("nosfenxin")
    {
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (!TriggerSkill::triggerable(killer) || killer->getMark("@burnheart") == 0)
            return false;
        player->setFlags("NosFenxinTarget");
...
AssassinsPackage::AssassinsPackage() : Package("assassins")
{
    ...
    General *as_lingju = new General(this, "as_lingju", "qun", 3, false);
    as_lingju->addSkill("jieyuan");
    as_lingju->addSkill(new NosFenxin);
