        ...
        if (triggerEvent == CardsMoveOneTime && player->getPhase() == Player::Discard) {
            ...
            if (move.from == player && (move.reason.m_reason & CardMoveReason::S_MASK_BASIC_REASON) == CardMoveReason::S_REASON_DISCARD) {
                foreach (int card_id, move.card_ids) {
                    shejianRecord << card_id;
                }
            }
            player->tag["ShejianRecord"] = shejianRecord;
...
class Yinshi : public ProhibitSkill
{
public:
    Yinshi() : ProhibitSkill("yinshi")
    {
    }
...
class YinshiSkip : public PhaseChangeSkill
{
public:
    YinshiSkip() : PhaseChangeSkill("#yinshi-skip")
    {
    ...
    virtual bool onPhaseChange(ServerPlayer *player) const
    {
        ...
        player->getRoom()->sendCompulsoryTriggerLog(player, "yinshi");
        player->broadcastSkillInvoke("yinshi");
...
: Package("MOL")
{
    ...
    pangdegong->addSkill(new Yinshi);
    pangdegong->addSkill(new YinshiSkip);
    related_skills.insertMulti("yinshi", "#yinshi-skip");
