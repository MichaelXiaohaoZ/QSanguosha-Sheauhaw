...
public:
    ...
    virtual void onDamaged(ServerPlayer *caocao, const DamageStruct &damage) const
    {
        ...
        if (room->isAllOnPlace(card, Player::PlaceTable)) choices.append("obtain");
		...
		if (triggerEvent == CardsMoveOneTime && TriggerSkill::triggerable(player)) {
            ...
            foreach (ServerPlayer *xiahou, room->getAllPlayers()) {
                if (xiahou->getPile("qingjian").length() > 0 && TriggerSkill::triggerable(xiahou)) {
					...
					do {
                        const Card *use = room->askForUseCard(xiahou, "@@qingjian_allot!", "@qingjian-give", QVariant(), Card::MethodNone);
...
public:
    ...
    virtual bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *zhenji, QVariant &data) const
    {
        if (triggerEvent == EventPhaseStart && zhenji->getPhase() == Player::Start) {
            ...
            bool first = true;
            QString name = "luoshen";
            while (zhenji->isAlive() && zhenji->askForSkillInvoke(name)) {
                if (first) {
    ...
    virtual bool trigger(TriggerEvent, Room *room, ServerPlayer *daqiao, QVariant &data) const
    {
        ...
        if (use.card->isKindOf("Slash") && use.to.contains(daqiao) && daqiao->canDiscard(daqiao, "he")) {
            ...
            if (can_invoke) {
                ...
                if (room->askForUseCard(daqiao, "@@liuli", prompt, QVariant(), Card::MethodDiscard)) {
...
void StandardPackage::addGenerals()
{
    ...
    skills << new NoDistanceTargetMod << new Xiaoxi << new NonCompulsoryInvalidity << new Jianyan << new GlobalFakeMoveSkill << new QingjianAllot << new RendeBasic;
}
