...
public:
    ...
    {
        return room->getMode() == "08_zdyj" && target != NULL && target->getMark("shown_loyalist");
    }
...
bool FenyueCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *) const
{
    ...
    : Package("BestLoyalistCard", Package::CardPack)
{
    ...
    skills << new ShowLord << new AllArmyDraw << new ByStove << new BeatAnotherGive << new TreasuredSwordSkill
